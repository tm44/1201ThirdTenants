<%@ Page Title="" Language="C#" MasterPageFile="~/master.Master" AutoEventWireup="true" CodeBehind="emergency-procedures.aspx.cs" Inherits="_1201ThirdTenants.your_safety.EmergencyProcedures" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <h1>Emergency Procedures</h1>
    <p>
        <img src="/images/your_safety.jpg" alt="Your Safety" class="img-responsive pull-right pageImage">Each Tenant will provide responsible personnel to assist the Property Management office and be responsible for controlling the movement and actions of Tenant employees during building and office emergencies.&nbsp; These assigned personnel are called Floor Wardens.&nbsp; An overview of their responsibilities is provided on the <a href="/your-safety/floor-wardens.aspx">Floor Warden</a> page.</p>
    <p>For general information on how to deal with specific emergencies, click on the headings below. If you would like a printed copy of the 1201 Third Avenue Emergency Response Manual, please contact the Property Management office.</p>
    <ul>
        <li><a href="/your-safety/emergency-procedures/bomb-threat.aspx" title="Bomb Threat">Bomb Threat</a></li>
        <li><a href="/your-safety/emergency-procedures/earthquake.aspx" title="Earthquake">Earthquake</a></li>
        <li><a href="/your-safety/emergency-procedures/building-evacuation.aspx" title="Building Evacuation">Building Evacuation</a></li>
        <li><a href="/your-safety/emergency-procedures/fire.aspx" title="Fire">Fire</a></li>
        <li><a href="/your-safety/emergency-procedures/power-outage.aspx" title="Medical Emergencies">Medical Emergencies</a></li>
        <li><a href="/your-safety/emergency-procedures/power-outage">Power Outage</a></li>
        <li><a href="/your-safety/emergency-procedures/suspicious-package.aspx" title="Suspicious Package">Suspicious Package</a></li>
        <li><a href="/your-safety/emergency-procedures/suspicious-or-hazardous-substance.aspx" title="Suspicious or Hazardous Substance">Suspicious or Hazardous Substance</a></li>
    </ul>
</asp:Content>
